...
uex  = @(x,y) y.^0.6+0*x;   %Analytical solution
uexx = @(x,y) 0*x.*y;   %Gradient Analytical solution (x)
uexy = @(x,y) 0.6*y.^(0.6-1)+0*x;   %Gradient Analytical solution (y)
ff   = @(x,y) 0.6*(0.6-1)*y.^(0.6-2)+0*x;   %Forcing term
g    = @(x,y) y.^0.6+0*x;   %Dirichlet condition
h    = @(x,y) 0*x.*y;   %Neumann condition
gam  = 0;                 %gamma value
...
figure(1);
hs = subplot(2,1,1);
loglog(h,h.^(-1/8),'-+b','Linewidth',2); hold on; grid on; 
